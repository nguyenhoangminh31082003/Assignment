...
class PaperfliesSupplier(Supplier):
  ...
  def parse_data(raw_data: dict) -> Hotel:
    ...
    roomImages = PaperfliesSupplier.parse_image_list(
        raw_data["images"]["rooms"])
    siteImages = PaperfliesSupplier.parse_image_list(
        raw_data["images"]["site"])
  ...
  def parse_image_list(raw_images: list[dict]) -> list[Image]:
    return [
        Image(link=image["link"], description=image["caption"])
